import { pgTable, pgSchema, uniqueIndex, index, foreignKey, check, uuid, text, timestamp, jsonb, boolean, varchar, bigserial, pgPolicy, smallint, json, inet, bigint, unique, integer, doublePrecision } from "drizzle-orm/pg-core"
...
export const chat = pgTable("Chat", {
	...
	unique("Chat_chat_id_key").on(table.chatId),
]);
...
export const rumour = pgTable("Rumour", {
	...
	foreignKey({
			...
			name: "Rumour_chat_table_id_fkey"
		}),
]);

export const rumourMatch = pgTable("RumourMatch", {
	id: uuid().defaultRandom().primaryKey().notNull(),
	similarity: doublePrecision(),
	count: integer().default(1),
	createdat: timestamp({ mode: 'string' }).defaultNow(),
	rumourid: uuid().notNull(),
}, (table) => [
	foreignKey({
			columns: [table.rumourid],
			foreignColumns: [rumour.id],
			name: "RumourMatch_rumourid_fkey"
		}).onDelete("cascade"),
]);

export const messageLog = pgTable("MessageLog", {
	id: uuid().defaultRandom().primaryKey().notNull(),
	chatTableId: uuid("chat_table_id"),
	...
	foreignKey({
			columns: [table.chatTableId],
			foreignColumns: [chat.id],
			name: "MessageLog_chat_table_id_fkey"
		}),
]);
